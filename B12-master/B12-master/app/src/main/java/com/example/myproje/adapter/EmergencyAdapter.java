...
public class EmergencyAdapter extends RecyclerView.Adapter<EmergencyAdapter.EmergencyHolder> {
    ...
    public void onBindViewHolder(@NonNull EmergencyHolder holder, int position) {
        Emergency emergency = emergencyArrayList.get(position);

        String name = (emergency.name != null) ? emergency.name : "Bilinmeyen";
        String number = (emergency.number != null) ? emergency.number : "-";

        holder.emergencyRowBinding.emergencyName.setText(name);
        holder.emergencyRowBinding.emergencyNumber.setText(number);
    }

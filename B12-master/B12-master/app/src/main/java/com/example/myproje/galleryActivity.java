    ...
    private void getData() {
        firebaseFirestore.collection("Photos").addSnapshotListener(new EventListener<QuerySnapshot>() {
            ...
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                ...
                if (value != null) {
                    postArrayList.clear(); // tekrar eklenmesin
                    for (DocumentSnapshot snapshot : value.getDocuments()) {
                        Map<String, Object> data = snapshot.getData();

                        if (data != null) {
                            String userEmail = data.get("useremail") != null ? data.get("useremail").toString() : "Bilinmiyor";
                            String comment = data.get("comment") != null ? data.get("comment").toString() : "";
                            String downloadUrl = data.get("downloadurl") != null ? data.get("downloadurl").toString() : "";
                            String imageName = data.get("imagename") != null ? data.get("imagename").toString() : "";
